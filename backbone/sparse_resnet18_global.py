...
class SparseBasicBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        out = kwinners2d_global(out, self.kw_percent_on,
                                self.kw_criterion, self.kw_relu)
...
class SparseResNet(nn.Module):
    ...
    def __init__(self, block: SparseBasicBlock, num_blocks: List[int],
        ...
        self._activation_counts['layer_1_classwise'] = torch.zeros(
            num_classes, nf * 1)
        self._activation_counts['layer_2_classwise'] = torch.zeros(
            num_classes, nf * 2)
        self._activation_counts['layer_3_classwise'] = torch.zeros(
            num_classes, nf * 4)
        self._activation_counts['layer_4_classwise'] = torch.zeros(
            num_classes, nf * 8)

        self.conv1 = conv3x3(3, nf * 1)
        self.bn1 = nn.BatchNorm2d(nf * 1)
        self.layer1 = self._make_layer(
            block, nf * 1, num_blocks[0], stride=1, kw_percent_on=kw_percent_on[0], kw_criterion=kw_criterion, kw_relu=kw_relu)
        self.layer2 = self._make_layer(
            block, nf * 2, num_blocks[1], stride=2, kw_percent_on=kw_percent_on[1], kw_criterion=kw_criterion, kw_relu=kw_relu)
        self.layer3 = self._make_layer(
            block, nf * 4, num_blocks[2], stride=2, kw_percent_on=kw_percent_on[2], kw_criterion=kw_criterion, kw_relu=kw_relu)
        self.layer4 = self._make_layer(
            block, nf * 8, num_blocks[3], stride=2, kw_percent_on=kw_percent_on[3], kw_criterion=kw_criterion, kw_relu=kw_relu)
    ...
    def _make_layer(self, block: SparseBasicBlock, planes: int,
        ...
        for stride in strides:
            layers.append(block(self.in_planes, planes, stride,
                          kw_percent_on, kw_criterion, kw_relu))
    ...
    def forward(self, x: torch.Tensor, y=None, disable_dropout=False, return_activations=False, mode='train', update_act_counts=True) -> torch.Tensor:
        ...
        for n, layer in enumerate(self._layers):
            ...
            if self.apply_heterogeneous_dropout and not disable_dropout and self.layerwise_dropout[n]:
                if self.training and self._keep_probs:
                    ...
                    idx = np.random.choice(
                        out.shape[1],
                        min(int(
                            self.kw_percent_on[n] * 1.1 * out.shape[1]), out.shape[1]),
                        p=prob,
                        replace=False
                    )
                    heterogeneous_mask[idx] = 1
                    heterogeneous_mask = (
                        heterogeneous_mask > 0).repeat(len(y), 1)

                    classwise_mask = torch.rand(
                        out.shape[1]) < \
                        self._keep_probs[f'layer_{n + 1}_classwise'][y.to("cpu")]

                    # If classwise probabilities available, use them instead use heterogeneous dropout
                    layer_mask = torch.where(
                        (self._keep_probs[f'layer_{n + 1}_classwise'].sum(1)[y.to("cpu")] > 0)[
                            :, None].repeat(1, heterogeneous_mask.shape[1]),
                        classwise_mask,
                        heterogeneous_mask
                    )
            ...
            # Apply Activation
            out = kwinners2d_global(
                out, self.kw_percent_on[n], self.kw_criterion, self.kw_relu)
            activations[f'layer_{n + 1}'] = out

            if self.training and update_act_counts:
                filter_activation = out.flatten(
                    start_dim=-2, end_dim=-1).sum(-1) > 0
                batch_act_counts = filter_activation.sum(0)
                self._activation_counts[f'layer_{n+1}'] += batch_act_counts.cpu()
                if y is not None:
                    for class_idx in range(self.num_classes):
                        sel_idx = y == class_idx
                        self._activation_counts[f'layer_{n + 1}_classwise'][class_idx] += (
                            filter_activation[sel_idx]).sum(dim=0).cpu()
