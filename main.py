...
from model.sparse_ser import SparseSER
from backbone.sparse_resnet18_global import sparse_resnet18
...
def get_args_parser():
    ...
    parser.add_argument('--root', type=str, default='~/.avalanche/data')
    ...
    # Sparse SER args
    parser.add_argument('--kw', default=[0.9, 0.9, 0.9, 0.8])
    ...
    parser.add_argument('--dropout_warmup', type=int, default=15)
...
def train(cl_model, train_loader, t, args):
    ...
    if args.dataset == 'cifar10':
        scheduler = torch.optim.lr_scheduler.MultiStepLR(
            cl_model.optim, [15], gamma=0.1)
    if args.dataset == 'cifar100':
        scheduler = torch.optim.lr_scheduler.MultiStepLR(
            cl_model.optim, [35, 45], gamma=0.1)
    if args.dataset == 'tinyimg':
        scheduler = torch.optim.lr_scheduler.MultiStepLR(
            cl_model.optim, [70, 90], gamma=0.1)
    ...
    for epoch in range(args.n_epochs):
        for batch_idx, data in enumerate(train_loader):
            ...
            progress_bar(batch_idx, len(train_loader),
                         t+1, epoch+1, loss.item())

        if scheduler is not None:
            scheduler.step()
        if hasattr(cl_model, 'end_epoch'):
            cl_model.end_epoch(epoch)
...
def evaluate(cl_model, test_splits, setting, args):
    ...
    for t in range(len(test_splits)):
        ...
        test_loader = DataLoader(
            test_splits[t], batch_size=args.batch_size, shuffle=False)
        ...
        for batch_idx, data in enumerate(test_loader):
            ...
            if setting == 'class_il':
                predict = filter_classes(
                    predict, t, args.n_classes, args.n_tasks)
                pred = torch.max(predict, 1)[1]
                correct = pred.eq(label.data).cpu().sum().numpy()
        ...
        accs.append(
            100 * np.sum(metrics['correct']) / np.sum(metrics['count']))
        if setting == 'class_il':
            accs_task.append(
                100 * np.sum(metrics_task['correct']) / np.sum(metrics_task['count']))
...
def main(args):
    ...
    if args.dataset in ['perm-mnist', 'rot-mnist']:
        ...
        setting = 'class_il'
        train_set = get_dataset(args.root, args.dataset,
                                train=True, transform=transforms.ToTensor())

        tf_test = get_transform(args.dataset, train=False)
        test_set = get_dataset(args.root, args.dataset,
    ...
    elif args.model == "sparse-ser":
        net = sparse_resnet18(
            nclasses=args.n_classes,
            kw_criterion=args.kw_criterion,
            kw_percent_on=args.kw).to(args.device)
        net.apply_heterogeneous_dropout = True
        net.layerwise_dropout = [0, 0, 0, 1]
        cl_model = SparseSER(net, args)
    ...
    for t in range(args.n_tasks):
        ...
        if setting == 'class_il':
            ...
            train_split = get_dataset(
                args.root, args.dataset, train=True, transform=transforms.ToTensor())
            test_split = get_dataset(
                args.root, args.dataset, train=False, transform=transform)
        ...
        if setting == 'class_il':
            ...
            print('\nclass-il: ', [float('{:.02f}'.format(d))
                  for d in accs[0]], round(np.mean(accs[0]), 2))
            print('task-il : ', [float('{:.02f}'.format(d))
                  for d in accs[1]], round(np.mean(accs[1]), 2))
        else:
            print('\ndomain-il: ', [float('{:.02f}'.format(d))
                  for d in accs[0]], round(np.mean(accs[0]), 2))
...
if __name__ == '__main__':
    ...
    save_path = os.path.join(
        'ckpt', f'{args.model}_{args.dataset}_{args.n_tasks}')
